...
SPDK_LINK_FLAGS := $(shell PKG_CONFIG_PATH="$(SPDK_PKG_CONFIG_PATH)" pkg-config --libs spdk_env_dpdk spdk_event spdk_bdev)
...
LINK_FLAGS := -Wl,--whole-archive -lpthread -lrt -lnuma -ldl -luuid -lm -lisal

all: reactor_demo

reactor_demo: clean
	$(CC) --std=c++11 reactor_demo.cc $(LINK_FLAGS) -o reactor_demo -Wl,--no-as-needed -Wl,--whole-archive $(DPDK_LINK_FLAGS) $(SPDK_LINK_FLAGS) -Wl,--as-needed
